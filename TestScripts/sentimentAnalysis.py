...
from transformers.models.siglip.convert_siglip_to_hf import model_name_to_image_size
...
def evaluate_predictions(true_ratings, predicted_ratings):
    ...
    return precision, pearson_corr, mse, mae, r2
...
def main():
    ...
    dataloader = DataLoader(dataset, batch_size=20)  # Adjust batch size based on your resources
    ...
    precision, pearson_corr, mse, mae, r2 = evaluate_predictions(true_ratings, predicted_ratings)

    # Visualize Results
    print("Visualizing results...")
    plot_distributions(true_ratings, predicted_ratings)

    output_file= " performance-"+(str.format(model_name)).replace("/","-")
    processed_reviews={
